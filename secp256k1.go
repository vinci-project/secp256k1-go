...
import (
	...
	secp "github.com/haltingstate/secp256k1-go/secp256k1-go2" // Reset to our path
)
...
func pubkeyFromSeckey(seckey []byte) []byte {
	...
	if ret := secp.PubkeyIsValid(pubkey); ret != 1 {
		log.Panic("ERROR: pubkey invald, ret: ", ret)
		return nil
	}
	...
	if ret := VerifyPubkey(pubkey); ret != 1 {
		...
		log.Panic("ERROR: pubkey verification failed, for deterministic. ret: ", ret)
		return nil
	}
...
new_seckey:
	...
	if ret := secp.PubkeyIsValid(pubkey); ret != 1 {
		log.Panic("ERROR: Pubkey invalid, ret: ", ret)
		goto new_seckey
	}
...
//generates deterministic keypair with weak BLAKE2b 256 hash of seed
//internal use only
...
//double BLAKE2b 256, salted with ECDH operation in curve
func Secp256k1Hash(hash []byte) []byte {
	hash = SumBLAKE2b256(hash)
	_, seckey := generateDeterministicKeyPair(hash)                //seckey1 is usually BLAKE2b 256 of hash
	pubkey, _ := generateDeterministicKeyPair(SumBLAKE2b256(hash)) //SumBLAKE2b256(hash) equals seckey usually
	ecdh := ECDH(pubkey, seckey)                                   //raise pubkey to power of seckey in curve
	return SumBLAKE2b256(append(hash, ecdh...))                    //append signature to SumBLAKE2b256(seed) and hash
}
...
func Sign(msg []byte, seckey []byte) []byte {
	...
	msg1.SetBytes(SumBLAKE2b256(msg))
	...
	if len(sig_bytes) != 64 {
		log.Fatal("Invalid signature byte count: ", len(sig_bytes))
	}
...
func RecoverPubkey(msg []byte, sig []byte) []byte {
	...
	if ret != 1 {
		log.Println("RecoverPubkey: code: ", ret)
		return nil
	}
